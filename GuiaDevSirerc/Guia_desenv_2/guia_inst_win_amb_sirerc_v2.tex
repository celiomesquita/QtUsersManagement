...
Este manual foi atualizado por meio de uma máquina com \textit{Windows 11 Pro}.

\subsection{\textit{Download} e instalação da última versão do \qt{}}
...
Como, no passo anterior, não é mais possível optar pelo \qtfive{}, nos resta a opção da instalação a versão \textit{offline}.
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.95\textwidth]{images/qt_offline_installer}}
	\caption{\download{} do instalador do Qt 5.15.x }
	\label{fig:qt_offline_installer}
\end{figure}	

Salve o arquivo compactado e continue com a instalação do \qtcreator{}. Posteriormente iremos demonstrar a instalação do \qtfive{}.

\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.75\textwidth]{images/qt_everywhere}}
	\caption{Salve o arquivo compactado. }
	\label{fig:qt_everywhere}
\end{figure}	
...
\section{INSTALAÇÃO DO QT 5.15.2 NO WINDOWS}

\subsection{Requisitos do Sistema}

\begin{itemize}
	\item Perl 5.8 ou posterior
	\item Python 2.7 ou posterior
	\item Compilador C++ com suporte ao padrão C++11
\end{itemize}

Para outros requisitos específicos da plataforma, consulte a seção "Configurando sua máquina" em: \href{http://wiki.qt.io/Get_The_Source}{http://wiki.qt.io/Get\_The\_Source}
...
\begin{verbatim}
	...
	* Compilador suportado (Visual Studio 2012 ou posterior,
	MinGW-builds gcc 4.9 ou posterior)
...
Para acelerar o bootstrap do qmake com MSVC, pode ser útil passar \texttt{"-make-tool jom"} na linha de comando do \texttt{configure}. Se você não usar \texttt{jom}, adicionar \texttt{"/MP"} à variável de ambiente \texttt{CL} é uma boa ideia.

\subsection{BUILD!}

Um processo típico de `\texttt{configure; make}' é usado.

\subsection*{Algumas opções relevantes do configure (veja \texttt{configure -help}):}

\begin{itemize}
	\item \texttt{-release} \hspace{3pt} Compila e vincula o Qt com a depuração desativada.
	\item \texttt{-debug} \hspace{3pt} Compila e vincula o Qt com a depuração ativada.
	\item \texttt{-nomake tests} \hspace{3pt} Desativa a construção de testes para acelerar a compilação.
	\item \texttt{-nomake examples} \hspace{3pt} Desativa a construção de exemplos para acelerar a compilação.
	\item \texttt{-confirm-license} \hspace{3pt} Reconhece automaticamente a licença LGPL 2.1.
\end{itemize}

\subsection*{Exemplo para uma construção de release:}
...
\subsection*{Exemplo para uma construção de desenvolvedor:}
...
Veja a saída de \texttt{./configure -help} para a documentação sobre várias opções para configurar.

Os exemplos acima irão construir quaisquer módulos Qt5 que tenham sido habilitados por padrão no sistema de construção.

É possível construir módulos selecionados com suas dependências executando um \texttt{make module-<foo>}. Por exemplo, para construir apenas \texttt{qtdeclarative}, e os módulos dos quais ele depende:
...
O repositório de submódulos \texttt{qtrepotools} contém scripts úteis para desenvolvedores e engenheiros de liberação. Considere adicionar \texttt{qtrepotools/bin} à sua variável de ambiente \texttt{PATH} para acessá-los.

A ferramenta \texttt{qt5\_tool} no \texttt{qtrepotools} possui alguns recursos adicionais que podem ser de interesse. Experimente \texttt{qt5\_tool --help}.

\section*{Construindo o Qt5 a partir do git}

1. Install Required Tools
Make sure you have the following tools installed:

Visual Studio: Qt recommends using a specific Visual Studio version for compatibility. For \qtfive{}, you can use Visual Studio 2017 or 2019. During installation, ensure the C++ development components are installed. 

\url{https://visualstudio.microsoft.com/pt-br/vs/older-downloads/}
...
Ruby: Download and install Ruby from rubyinstaller.org.
...
2. Download \qtfive{} Source

If you haven't already, you can download the \qtfive{} source code from the official Qt website or use the version you've cloned.
...
Now you're all set to proceed with the compilation process. You can follow the steps to configure and build \qtfive{} as mentioned earlier
...
Abra um terminal no \textbf{Prompt de Comando do Desenvolvedor} Visual Studio.

\begin{verbatim}
	configure.bat -prefix "C:\Users\aluno\Documents\qt5\Qt5.15.2" -platform win32-msvc2019 -release -opensource -confirm-license -nomake examples -nomake tests -skip qtwebengine
\end{verbatim}

-prefix \begin{verbatim}"C:\...\qt5\end{verbatim}: The directory where Qt will be installed after building.
-platform win32-msvc2019: Specifies Visual Studio 2019 for building.
-release: Build a release version.
-opensource: Use the open-source license.
-confirm-license: Automatically confirm the open-source license agreement.
-nomake examples -nomake tests: Skip building examples and tests to speed up the process.
-skip qtwebengine: Skips the Qt WebEngine module, which can be complex to build.


The -prefix option in the configure.bat command specifies where Qt will be installed after the build process, so the directory \begin{verbatim}C:\Users\aluno\Documents\qt5\Qt5.15.2\end{verbatim} doesn't need to exist yet. It will be created during the installation step (nmake install) if it doesn't exist.

Here's what happens:
-prefix \begin{verbatim}C:\Users\aluno\Documents\qt5\Qt5.15.2\end{verbatim}: This tells Qt where to install its built binaries, libraries, and tools once the build process is complete.
Qt will automatically create the \qtfive{} directory during the installation phase.
What you should do:
You don’t need to manually create the directory. When the nmake install command is executed after the build, it will place the files in the specified -prefix directory. So you can safely proceed with the configure.bat command as it is.

To summarize:

Run the \begin{verbatim}init-repository\end{verbatim} command to initialize the submodules.
Use the same configure.bat command with the -prefix option pointing to \begin{verbatim}C:\Users\aluno\Documents\qt\Qt5.15.2\end{verbatim}.
The \qtfive{} folder will be created automatically during the installation phase.


Run vcvarsall.bat with the Correct Arguments
Open the Visual Studio Developer Command Prompt or use the current command prompt where you're running the script.
...
Once you have set up the environment, you can proceed with your Qt build:
